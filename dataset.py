...
class PianoRollAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        show_1= result['audio'].cpu().numpy()
        show_2= result['onset'].cpu().numpy()
        show_3= result['offset'].cpu().numpy()
        show_4= result['frame'].cpu().numpy()
        show_5= result['velocity'].cpu().numpy()
...
class MAPS(PianoRollAudioDataset):
    ...
    def files(self, group):
        flacs = glob(os.path.join(self.path, 'flac', '*_%s.wav' % group))
        tsvs = [f.replace('.wav', '.tsv') for f in flacs]
        tsvs = [f.replace('flac', 'tsv\matched') for f in tsvs]
